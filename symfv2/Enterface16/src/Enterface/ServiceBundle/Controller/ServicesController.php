...
public function algotreatment1Action()
    ...
    public function algotreatment2Action()
    {
        ...
        $time_exec=fgets($myfile );
        //return new RedirectResponse($this->container->get('router')->generate('enterface_service_showtracking'));
        return $this->render('EnterfaceServiceBundle:Services:affichealgo.html.twig', array('works' => true,'algo'=>$algorithm,'type'=>$typealgo,'mode'=>$mode,'time_exec'=>$time_exec));
    }
   ...
   $stream2 = ssh2_exec($connection, 'cmd /C java -jar Downloads\ftp\BASIC_CLEO_V5A.jar Downloads\ftp\dicom\fichier0.dcm Downloads\ftp\Input\BMDvalues1.txt Downloads\ftp\Input\Phantom1.txt');
    ...
    public function cleotreatmentadvAction() {
     ...
     if (ssh2_auth_password($connection, $ftp_user_name, $ftp_user_pass))
     {       
     $stream2 = ssh2_exec($connection, 'cmd /C java -jar Downloads\ftp\ADVANCED_CLEO_V5.jar Downloads\ftp\dicom\fichier0.dcm Downloads\ftp\Input\BMDvalues1.txt Downloads\ftp\Input\Phantom1.txt');
                 stream_set_blocking($stream2, true);
                 stream_get_contents($stream2);
                 $s1=ssh2_exec($connection, 'cmd /C move C:\Users\mohamedamine_belarbi\BMD.txt C:\Users\mohamedamine_belarbi\Downloads\ftp');
                 stream_set_blocking($s1, true);
    ...
    public function cleotreatmentpersoAction() {
     ...
     ftp_put( $connect_it,"Input\BMDvalues1.txt",$target_bmd, FTP_BINARY );
     ftp_put( $connect_it, "Input\Phantom1.txt",$target_phantom, FTP_BINARY );
     ftp_put( $connect_it, "Input\parameters.txt",$target_parameters, FTP_BINARY );
     ...
     if($var1==1)
     {
       $image2="$outputdir/image2.gif";
       $image3="$outputdir/image3.gif";
       $bmd3="$outputdir/Results1.txt";
       ftp_get($connect_it, $bmd3, "Results/CLEO_Results_Personalized.txt", FTP_BINARY);
